    ...
    function rebuild() {
        ...
        for (const p of diff) {
            ...
            if (p.removed) {
                for (const t of arr) {
                    ln++;
                    lp.push({ text: t, num: ln, removed: true });
                }
            } else if (p.added) {
                for (const t of arr) {
                    rn++;
                    rp.push({ text: t, num: rn, added: true });
                }
            } else {
                for (const t of arr) {
                    ln++;
                    rn++;
                    lp.push({ text: t, num: ln });
                    rp.push({ text: t, num: rn });
                }
            }
        }
        leftPart = lp;
    ...
    {#if left || right}
        <div class="mt-4 flex gap-2">
            <div class="w-1/2 overflow-auto font-mono text-xs leading-relaxed">
                <div
                    class="rounded-t border-x border-t border-brand-secondary bg-white/5 p-1 px-2 text-[10px] text-brand-text"
                >
                    Original
                </div>
                <div class="border border-brand-secondary bg-white/5 p-2">
                    {#each leftPart as l (l.num)}
                        <div class="flex">
                            <span
                                class="mr-2 w-6 shrink-0 text-right text-brand-text/40 select-none"
                                >{l.num}</span
                            >
                            <span
                                class="break-all whitespace-pre-wrap {l.removed
                ...
                <div class="border border-brand-secondary bg-white/5 p-2">
                    {#each rightPart as l (l.num)}
                        <div class="flex">
                            ...
                            >
                            <span
                                class="break-all whitespace-pre-wrap {l.added

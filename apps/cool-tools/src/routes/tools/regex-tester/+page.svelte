...
<ToolShell title="Regex Tester" desc="Test regular expressions with live matching." max="max-w-3xl">
    <div class="mb-3 flex flex-wrap items-center gap-3">
        <label class="text-sm text-brand-text" for="pattern"
            >/<input
                id="pattern"
                type="text"
                placeholder="pattern"
    ...
    {#if matches.length > 0}
        ...
        <div class="flex flex-col gap-1.5">
            {#each matches as m, i (m.index)}
                <div
                    class="rounded border border-brand-secondary bg-white/5 px-3 py-2 font-mono text-xs text-brand-text-highlight"
                >
                    ...
                    {#if m.groups.length > 0}
                        <div class="mt-1 pl-4 text-brand-text">
                            {#each m.groups as g, gi (gi)}
                                <div>
                                    Group {gi + 1}:
                                    <span class="text-brand-text-highlight"
                                        >{g ?? '(undefined)'}</span
                                    >
                                </div>
                            {/each}

<script lang="ts">
    ...
    let groups = $state<Group[]>([
        { id: 'default', name: 'Untitled Group', people: [], expenses: [] }
    ]);
    ...
    function addPerson() {
        ...
        groups = groups.map((grp) =>
            grp.id === activeGroup
                ? { ...grp, people: [...grp.people, { name: n, id: crypto.randomUUID() }] }
                : grp
        );
        newName = '';
        save();
    ...
    function finishRenamePerson(id: string) {
        ...
        groups = groups.map((grp) =>
            grp.id !== activeGroup
                ? grp
                : { ...grp, people: grp.people.map((p) => (p.id === id ? { ...p, name: n } : p)) }
        );
        editingPerson = null;
        save();
    }
    ...
    function name(id: string) {
        return g.people.find((p) => p.id === id)?.name ?? '?';
    }
    ...
    function importGroup() {
        ...
        input.onchange = async (e) => {
            ...
            try {
                ...
                /* invalid/unsupported file, ignore */
            }
        };
        input.click();
    }
...
<ToolShell
    title="Settle Up"
    desc="Split expenses and find the minimum transfers to settle debts. Data is saved locally."
>
    <h3>Groups</h3>
        ...
        {#each groups as grp (grp.id)}
            {#if editingGroup === grp.id}
                <input
                    ...
                    onkeydown={(e) => e.key === 'Enter' && finishRenameGroup(grp.id)}
                    onblur={() => finishRenameGroup(grp.id)}
                    class="w-24 text-xs"
                />
            {:else}
                <span
                    class="flex cursor-pointer items-center gap-1 rounded border px-2 py-1 text-xs transition-colors {grp.id ===
                    activeGroup
                        ? 'border-brand-primary bg-white/10 text-brand-text-highlight'
                        : 'border-brand-secondary bg-white/5 text-brand-text-highlight hover:bg-white/10'}"
                    role="button"
                    tabindex="0"
                ...
                >
                    {grp.name}
                    <button
                        class="border-0 bg-transparent p-0 text-brand-text/60 hover:text-brand-text-highlight"
                        onclick={(e) => {
                            e.stopPropagation();
                            startRenameGroup(grp.id);
                        }}
                        title="Rename">~</button
                    >
                    {#if groups.length > 1}
                        <button
                            class="border-0 bg-transparent p-0 text-red-400/70 hover:text-red-400"
                            onclick={(e) => {
                                e.stopPropagation();
                                removeGroup(grp.id);
                            }}
    ...
    {#if g.people.length > 0}
        <div class="mb-4 flex flex-wrap gap-2">
            {#each g.people as p (p.id)}
                <span
                    class="flex items-center gap-1 rounded border border-brand-secondary bg-white/5 px-2 py-1 text-xs text-brand-text-highlight"
                >
                    {#if editingPerson === p.id}
                        <input
                            type="text"
                            bind:value={editingPersonName}
                            onkeydown={(e) => e.key === 'Enter' && finishRenamePerson(p.id)}
                            onblur={() => finishRenamePerson(p.id)}
                            class="w-20 text-xs"
                        />
                    {:else}
                        {p.name}
                        <button
                            class="border-0 bg-transparent p-0 text-brand-text/60 hover:text-brand-text-highlight"
                            onclick={() => startRenamePerson(p.id)}
                            title="Rename">~</button
                        >
                        <button
                            class="border-0 bg-transparent p-0 text-red-400/70 hover:text-red-400"
    ...
    {:else}
        <p class="mb-6 text-sm text-brand-text">
            No people yet — add someone above to start splitting expenses.
        </p>
    {/if}
        ...
        {#if g.expenses.length > 0}
            ...
            <div class="mb-4 flex flex-col gap-1">
                {#each g.expenses as e, i (i)}
                    <div
                        class="rounded border bg-white/5 transition-colors {openSplit === i
                    ...
                    >
                        ...
                        >
                            <span class="min-w-0">
                                <span class="font-semibold text-brand-text-highlight"
                                    >{name(e.paidBy)}</span
                                >
                                paid
                                <span class="font-semibold text-brand-text-highlight"
                                    >{e.currency} {e.amount.toFixed(2)}</span
                                >
                                {#if e.desc}<span class="text-brand-text"> — {e.desc}</span>{/if}
                            </span>
                            <button
                                class="ml-2 size-5 shrink-0 p-0 text-[10px]"
                                onclick={(ev) => {
                                    ev.stopPropagation();
                                    removeExpense(i);
                                }}>✕</button
                        ...
                        {#if openSplit === i}
                            <div
                                class="rounded-b border-t border-brand-secondary/60 px-3 py-1.5 text-xs text-brand-text"
                                transition:slide|local
                            >
                                Split among: <span class="text-brand-text-highlight"
                                    >{e.splitAmong.map(name).join(', ')}</span
                                >
                            </div>
                        {/if}
                    </div>
        ...
        {#if txfr.length > 0}
            ...
            <div class="flex flex-col gap-1">
                {#each txfr as t (t.from + t.to)}
                    <div
                        class="rounded border border-green-400/30 bg-green-400/5 px-3 py-1.5 text-sm text-green-300"
                    >
                        {name(t.from)} → {name(t.to)}:
                        <span class="font-bold"
                            >{displayCurrency} {(t.amount * transferRate).toFixed(2)}</span
                        >
                    </div>
                {/each}
            </div>

<script lang="ts">
    ...
    const dialStyle =
        'max-w[clamp(5rem,18vh,10rem)] aspect-square w-full rounded-full shadow-indent';
</script>

<aside class="flex h-full flex-col justify-between">
    <div class="my-[clamp(0.5rem,5vh,3rem)]">
    ...
    <div class="mt-20 flex h-full flex-col items-center gap-[clamp(0.5rem,100rem,4rem)]">
        <div class="flex w-full flex-row items-center gap-[clamp(0.25rem,2vh,2.5rem)]">
            <div class={dialStyle}>
                <SectionDial />
            </div>
            <div class={dialStyle}>
